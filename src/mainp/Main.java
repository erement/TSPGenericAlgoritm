...
public class Main
{
	...
	private static int[]
	mc[]= new int[2*populationN][populationN],//основной массив попул€ций
	mg[]=new int[2*populationN][populationN],//массив временного хранени€ попул€ций
	
	ideal=new int[populationN];//массив с лучшей особью(маршрутом)
	private static void parseWay(){
		//превращение координат городов в рассто€ние между ними.
		int h,g,z,z1=-1,z2=0;
	...
	private static void orderLiness(){
		//выставление пор€дка следовани€ по городам
		int x=1;
		for(int i=0;i<populationN/2; i++)
		    for(int j=0;j<populationN; j++)
			if (mop.getcity(i,j)>0){
	...
	private static int[][] initGert(){
		//здесь жесть #1. ‘ормирование начальной 
		//попул€ции маршрутов
		int w=0,r=0,h=0,
	...
	private static void mutation(int[] a){
		//мутаци€ случайного маршрута путем переставлени€
	...
	{//основа.—читывание количества проходов по алгоритму,
